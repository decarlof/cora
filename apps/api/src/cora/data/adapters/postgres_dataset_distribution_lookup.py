...
`proj_data_distribution_summary` for every non-Discarded Distribution of a
given Dataset (any status) so the decider can both gate on Verified AND
diagnose a Stale-only input distinct from a no-Distribution input.
...
class PostgresDatasetDistributionLookup:
    """Reads `proj_data_distribution_summary` for a Dataset's non-Discarded rows."""
    ...
    async def find_by_dataset(
        ...
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(_LOOKUP_SQL, dataset_id)
        return tuple(
            DatasetDistributionLookupResult(

...
Two loose families that were used only at 4-ID reach their second sighting at i10. The promotion guard (`PROMOTION_THRESHOLD = 2`) makes a loose family used by two or more deployments require a recorded hold-or-graduate decision: the signal is mechanical, the decision stays human. At the time this scaffold was written both were recorded **hold**, since a second sighting is not yet the rule-of-three. `Magnet` has since graduated: i10-1 was one of the three consumers (with 4-ID and ESRF ID32) whose rule-of-three earned it into the catalog, so i10's magnets now bind the graduated catalog `Magnet` Family (it presents the `Regulator` Role, the field a settable process variable). `PolarizationAnalyzer` stays held, still short of a genuine third, independent sighting.
...
| `Magnet` | 4-ID, i10 (i10-1), ID32 | the i10-1 electromagnet and the superconducting field-sweep magnet | **graduated**: i10-1 was one of the three consumers (4-ID + i10-1 + ID32) whose rule-of-three earned it; both magnet devices are one Family, the field-sweep capability is a per-Asset bound-Model affordance, not a split (the `InsertionDevice` / `TemperatureController` precedent); presents the `Regulator` Role (`MAG-1` now covers only the per-Asset field detail) |

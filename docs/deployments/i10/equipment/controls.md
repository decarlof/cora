...
Because there is no area detector, there is no pixel-array file-writing seam to model; the point channels are the science signal. See [Open questions](../questions.md) for the control, detection, and safety items still to confirm, and [Model](../model.md#deliberately-not-here-yet) for the deferred Method decisions, the held-under-review loose `PolarizationAnalyzer` on the POLAN arm (POL-2), and the graduated `Magnet` shared by both magnet devices (MAG-1).

...
Neither side coins a Family here. The diffractometer reuses `Goniometer`, the cryostat stage and the magnet stages reuse `LinearStage`, the Lakeshores reuse the graduated `TemperatureController`, the pinhole reuses `Aperture`, and the point detection reuses `FluxMonitor`. The polarization-analysis arm binds the loose `PolarizationAnalyzer`, its second sighting after 4-ID, held under review and not graduated here. The two magnet devices bind the graduated catalog `Magnet` Family: i10-1 was its second sighting after 4-ID, and with the later ESRF ID32 magnet the family reached a rule-of-three and graduated (see [Loose families at a second sighting](#loose-families-at-a-second-sighting)). The genuinely new modelling move at i10, polarization as a driven axis, sits on the source side as a reused `PseudoAxis`, not here (see the generated [Source walk](../beamline.md) and [Model](../model.md)).
...
Two families on this page were at their second sighting when this scaffold was written: `PolarizationAnalyzer` (the RASOR analysis arm, after 4-ID, POL-2) and `Magnet` (the i10-1 electromagnet and superconducting magnet, after 4-ID, MAG-1). Each was first seen at 4-ID and bound to a real device so the modelling is honest about what the endstation does.

`PolarizationAnalyzer` stays loose: graduation follows a rule-of-three across deployments, and a second sighting does not meet it, so i10 records a hold (POL-2). This mirrors how the fleet carries `StorageRing` loose at i22 until a confirmed device and a naming review settle it. `Magnet` has since graduated: i10-1's second sighting, together with the later ESRF ID32 magnet, completed its rule-of-three, so the i10-1 magnets now bind the graduated catalog `Magnet` Family (MAG-1 now covers only the per-Asset field detail). Everything else on this page reuses a catalog Family unchanged.

...
Families reused from the catalog: `InsertionDevice`, `Monochromator`, `Slit`, `Window`, `Mirror`, `Filter`, `Table`, `Camera`, `GenericProbe`, `Transfocator` (the CRL focusing optic), `Goniometer` (the diffractometer sample circles), `Magnet` (the sample magnets, graduated on the 4-ID + i10-1 + ID32 rule-of-three, presents `Regulator`), and `BeamPositionMonitor` (the beam-position monitors, a catalog Family presenting the `Sensor` Role, earned across the wide fleet that shares it, distinct from `FluxMonitor` by measuring beam position rather than flux). Bound to loose Family strings, held for gate-review: `PhaseRetarder`, `PolarizationAnalyzer`, `Laser` (single beamline). `Transfocator`, `TemperatureController`, `Magnet`, and `BeamPositionMonitor` recurred too and have since graduated to catalog Families (`TemperatureController` and `Magnet` present `Regulator`, `BeamPositionMonitor` presents `Sensor`). The loose ones are earned into the catalog only when a confirmed device registers and a naming review accepts the name; the graduation plan is on [Model](model.md#loose-family-graduation).

...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | SpectrometerArm graduated (earned across SIX + ID32 RIXS/XES + ID28); Magnet + PolarizationAnalyzer reached a rule-of-three and stay held (below) |
...
- **The remaining graduations (`MAG-1`, `POL-2`).** Held loose; each graduation (catalog.yaml family + naming-r3 + gate-review) is a dedicated follow-on PR. `SpectrometerArm` (`RIXS-1`) has since graduated.

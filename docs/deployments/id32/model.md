...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | SpectrometerArm graduated (earned across SIX + ID32 RIXS/XES + ID28) and Magnet graduated (earned across 4-ID + i10-1 + ID32; presents Regulator); PolarizationAnalyzer reached a rule-of-three and stays held (below) |
...
- **The remaining graduation (`POL-2`).** `PolarizationAnalyzer` stays held loose; its graduation (catalog.yaml family + naming-r3 + gate-review) is a dedicated follow-on PR. `SpectrometerArm` (`RIXS-1`) and `Magnet` (`MAG-1`) have since graduated.

...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | **none graduated here.** Cristallina reuses catalog Families and the graduated `Diffractometer` Assembly, `TemperatureController` Family, and `Magnet` Family (a further consumer of the graduated `Magnet`); loose families (`Laser`, `Diagnostic`, `FluxMonitor`) reused |
...
### The vector magnet is a fourth `Magnet` consumer, held (MAG-1)
